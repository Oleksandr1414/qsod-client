export const recipes = {
  1: {
    id: 1,
    name: "Pasta with Tomato Sauce",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",
    description: "Delicious pasta with aromatic tomato sauce.",
    ingredients: [
      { name: "pasta", quantity: "200 g" },
      { name: "tomatoes", quantity: "4 pieces" },
      { name: "olive oil", quantity: "2 tbsp" },
      { name: "garlic", quantity: "2 cloves" },
      { name: "spices", quantity: "to taste" },
    ],
    formula:
      "Cook the pasta, then prepare the sauce with tomatoes, garlic, and spices. Mix it with the pasta and serve hot.",
  },
  2: {
    id: 2,
    name: "Caesar Salad",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Classic Caesar salad with chicken.",
    ingredients: [
      { name: "chicken", quantity: "300 g" },
      { name: "lettuce", quantity: "1 head" },
      { name: "croutons", quantity: "50 g" },
      { name: "parmesan", quantity: "50 g" },
      { name: "Caesar dressing", quantity: "3 tbsp" },
    ],
    formula:
      "Cook the chicken, mix it with lettuce, croutons, and parmesan. Add Caesar dressing and toss until well coated.",
  },
  3: {
    id: 3,
    name: "Grilled Salmon",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Juicy grilled salmon with lemon and herbs.",
    ingredients: [
      { name: "salmon fillet", quantity: "2 pieces" },
      { name: "lemon", quantity: "1 piece" },
      { name: "olive oil", quantity: "2 tbsp" },
      { name: "dill", quantity: "1 tbsp" },
      { name: "salt and pepper", quantity: "to taste" },
    ],
    formula:
      "Season salmon with salt, pepper, and dill. Grill until cooked. Squeeze lemon juice and drizzle olive oil before serving.",
  },
  4: {
    id: 4,
    name: "Vegetarian Stir-Fry",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Colorful and nutritious vegetarian stir-fry.",
    ingredients: [
      { name: "tofu", quantity: "200 g" },
      { name: "broccoli", quantity: "1 cup" },
      { name: "carrots", quantity: "2 pieces" },
      { name: "bell peppers", quantity: "assorted colors, 1 cup" },
      { name: "soy sauce", quantity: "3 tbsp" },
    ],
    formula:
      "Stir-fry tofu and vegetables in soy sauce until tender. Serve hot.",
  },
  5: {
    id: 5,
    name: "Homemade Pizza",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Create your own pizza with your favorite toppings.",
    ingredients: [
      { name: "pizza dough", quantity: "1 ball" },
      { name: "tomato sauce", quantity: "1/2 cup" },
      { name: "mozzarella cheese", quantity: "1 cup" },
      { name: "pepperoni", quantity: "1/2 cup" },
      { name: "olives", quantity: "sliced, 1/4 cup" },
    ],
    formula:
      "Roll out the pizza dough, spread tomato sauce, add cheese and toppings. Bake until crust is golden and cheese is melted.",
  },
  6: {
    id: 6,
    name: "Chicken Noodle Soup",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Comforting chicken noodle soup for any day.",
    ingredients: [
      { name: "chicken broth", quantity: "4 cups" },
      { name: "chicken breast", quantity: "1 piece" },
      { name: "carrots", quantity: "2 pieces" },
      { name: "celery", quantity: "2 stalks" },
      { name: "egg noodles", quantity: "1 cup" },
    ],
    formula:
      "Boil chicken in broth, add vegetables and noodles. Simmer until chicken is cooked and flavors meld. Serve hot.",
  },
  7: {
    id: 7,
    name: "Shrimp Scampi",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Garlicky shrimp scampi with a hint of lemon.",
    ingredients: [
      { name: "shrimp", quantity: "400 g" },
      { name: "spaghetti", quantity: "200 g" },
      { name: "garlic", quantity: "4 cloves" },
      { name: "butter", quantity: "3 tbsp" },
      { name: "lemon zest", quantity: "1 tsp" },
    ],
    formula:
      "Sauté shrimp and garlic in butter, toss with cooked spaghetti. Add lemon zest and serve.",
  },
  8: {
    id: 8,
    name: "Caprese Salad",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description:
      "Light and refreshing Caprese salad with tomatoes and mozzarella.",
    ingredients: [
      { name: "tomatoes", quantity: "3 pieces" },
      { name: "mozzarella cheese", quantity: "200 g" },
      { name: "fresh basil", quantity: "1 cup" },
      { name: "balsamic glaze", quantity: "2 tbsp" },
    ],
    formula:
      "Slice tomatoes and mozzarella, arrange with fresh basil. Drizzle balsamic glaze before serving.",
  },
  9: {
    id: 9,
    name: "Beef Stir-Fry",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Quick and flavorful beef stir-fry with vegetables.",
    ingredients: [
      { name: "beef strips", quantity: "300 g" },
      { name: "broccoli", quantity: "1 cup" },
      { name: "bell peppers", quantity: "assorted colors, 1 cup" },
      { name: "soy sauce", quantity: "3 tbsp" },
      { name: "sesame oil", quantity: "1 tbsp" },
    ],
    formula:
      "Stir-fry beef and vegetables in soy sauce and sesame oil until beef is cooked and vegetables are tender. Serve hot.",
  },
  10: {
    id: 10,
    name: "Chocolate Chip Cookies",
    image_url:
      "https://images.immediate.co.uk/production/volatile/sites/2/2017/07/courgettes.jpg?quality=90&resize=556,505",

    description: "Classic chocolate chip cookies for a sweet treat.",
    ingredients: [
      { name: "butter", quantity: "1 cup" },
      { name: "brown sugar", quantity: "1 cup" },
      { name: "white sugar", quantity: "1/2 cup" },
      { name: "eggs", quantity: "2 pieces" },
      { name: "chocolate chips", quantity: "2 cups" },
    ],
    formula:
      "Cream butter and sugars, add eggs and mix. Stir in chocolate chips. Drop spoonfuls onto a baking sheet and bake until golden brown.",
  },
};
